...
function validateNavigation(items, label) {
    for (const item of items) {
        if (!item.label.trim())
            throw new TypeError(`${label} labels must not be empty.`);
        assertSafeUrl(item.to);
    }
}
export function validateStorefrontConfig(config) {
    if (!config.brand.name.trim())
        throw new TypeError('Brand name must not be empty.');
    if (!config.locale.trim())
        throw new TypeError('Locale must not be empty.');
    ...
    if (!/^[a-z0-9][a-z0-9._-]{0,63}$/iu.test(config.storageNamespace)) {
        throw new TypeError('Storage namespace must contain only letters, numbers, dots, underscores, or dashes.');
    }
    ...
    if (config.brand.logoUrl)
        assertSafeUrl(config.brand.logoUrl);
    ...
    for (const link of config.support)
        assertSafeUrl(link.url);
    assertUnique(config.catalog.products.map((product) => product.id), 'Product');
    assertUnique(config.catalog.categories.map((category) => category.id), 'Category');
    assertUnique(config.paymentMethods.map((method) => method.id), 'Payment method');
    ...
    for (const product of config.catalog.products) {
        if (!categoryIds.has(product.categoryId)) {
            throw new TypeError(`Product ${product.id} references unknown category ${product.categoryId}.`);
        }
    }
    if (typeof config.checkoutClient.startCheckout !== 'function' ||
...
export function defineStorefrontConfig(config) {
