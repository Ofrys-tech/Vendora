...
function decodeHtmlEntities(value: string): string {
  return value.replace(/&(#x[0-9a-f]+|#\d+|amp|colon|gt|lt|quot);?/giu, (entity, body: string) => {
    const normalized = body.toLowerCase();
    if (normalized.startsWith('#')) {
      const codePoint = Number.parseInt(
        normalized.slice(normalized.startsWith('#x') ? 2 : 1),
...
export function isSafeUrl(value: string, allowRelative = true): boolean {
  ...
  try {
    ...
    if (parsed.origin === 'https://storefront.invalid') return allowRelative;
...
export function assertSafeUrl(value: string, allowRelative = true): string {
  if (!isSafeUrl(value, allowRelative)) throw new TypeError(`Unsafe URL: ${value}`);
  return value;
}
...
function sanitizeTag(source: string): string {
  ...
  if (!opening) return escapeHtml(source);
  ...
  while (cursor < attributes.length) {
    ...
    if (!match) {
      if (attributes.slice(cursor).trim()) return '<a>';
      break;
    }
    cursor = attributePattern.lastIndex;
    const name = match[1]!.toLowerCase();
    const value = match[2] ?? match[3] ?? '';
    if (name === 'href' && isSafeUrl(value)) safeAttributes.push(`href="${escapeHtml(value)}"`);
    if (name === 'title') safeAttributes.push(`title="${escapeHtml(value)}"`);
  ...
  if (targetBlank) safeAttributes.push('rel="noopener noreferrer"');
